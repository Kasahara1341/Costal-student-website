...
- [サーバーpcの使い方](#サーバーpcの使い方)
  ...
  - [基本的な操作方法とルール](#基本的な操作方法とルール)
    - [基本的な操作方法](#基本的な操作方法)
    - [基本的なルール](#基本的なルール)
  - [Python仮想環境の作成と起動方法](#python仮想環境の作成と起動方法)
  - [サーバー上でのLatexによる執筆](#サーバー上でのlatexによる執筆)
    - [スニペット機能の利用](#スニペット機能の利用)
...
をsshに合わせると「新しいリモート(+マーク)」が選択できるのでクリックする．
...
初めて接続した際には自身のフォルダを作成すること(フォルダ名には英数字のみを使う)．またwindowsの場合，接続先でフォルダを開くたびにパスワードを入力する必要があるので注意する．
...
この機能を利用するには<span style="color:red">ローカルPCの</span>VS Codeの拡張機能に"LaTeX Workshop"が入っている必要がある．サーバーPCに接続しているVS Codeのウィンドウから拡張機能を開くと"LaTeX Workshop"が既に入っているように見えるため，ローカルPCのVS Codeにも入っていることを確認すること．
またVS Codeの設定に関しても編集が必要である．[VSCode で最高の LaTeX 環境を作る](https://qiita.com/rainbartown/items/d7718f12d71e688f3573)の「settings.jsonの編集」を参考にsetting.jsonを編集する必要がある．

諸々設定などができたら，texファイルを開いた際に下図のような<span style="color:green">三角のマーク</span>や窓に虫眼鏡をつけたようなマークが表示されるようになる．これらボタンからtexファイルからpdfファイルを作成や分割ウィンドウにpdfを表示することができる．
